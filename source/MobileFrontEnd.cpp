...
void MobileFrontEnd::DrawBackground(void *ecx0) {
    ...
    else if (FrontEndMenuManager.m_nCurrentMenuPage == MENUPAGE_KEYBOARD_CONTROLS) {
    ...
    if (FrontEndMenuManager.m_bShowMouse) {
        ...
        if (GetCursorPos(&position)) {
            if (FrontEndMenuManager.m_nCurrentMenuPage == 5 && CPad::NewMouseControllerState.lmb)
                FrontEndMenuManager.FrontEndSprites[24].Draw(position.x, position.y, SCREEN_MULTIPLIER(22.5f),
                                                             SCREEN_MULTIPLIER(22.5f), CRGBA(255, 255, 255, 255));
            else
                FrontEndMenuManager.FrontEndSprites[23].Draw(position.x, position.y, SCREEN_MULTIPLIER(22.5f), SCREEN_MULTIPLIER(22.5f), CRGBA(255, 255, 255, 255));
    ...
    for (unsigned int i = 0; i < 12; i++) {
        ...
        if (MenuPages[FrontEndMenuManager.m_nCurrentMenuPage].m_aButtons[i].m_nActionType != 1 && MenuPages[FrontEndMenuManager.m_nCurrentMenuPage].m_aButtons[i].m_szName[0]) {
            ...
            case 65:
                ...
                break;
            case 63:
                switch (FrontEndMenuManager.m_nPrefsRadioMode) {
                case 0:
            ...
            case 56:
            {
                const char* pszResolution = ((const char**(*)())0x745AF0)()[FrontEndMenuManager.m_nDisplayVideoMode];
            ...
            break;
            case 44:
                if (FrontEndMenuManager.m_nPrefsAntiAliasingDisp <= 1)
                    RightColumn = TheText.Get("FEM_OFF");
                else {
                    switch (FrontEndMenuManager.m_nPrefsAntiAliasingDisp)
                    {
                    case 2:
                        RightColumn = "2x";
                        break;
...
void __fastcall MobileFrontEnd::ProcessMobileMenuOptions(void *ecx0, int, signed char nArrowsInput, bool *bReturn, bool bEnterInput) {
    ...
    case 21:
        if (nArrowsInput >= 0)
        {
            if (FrontEndMenuManager.m_nStatsScrollDir)
            {
            ...
            else
            {
                ...
                FrontEndMenuManager.m_nSelectedMissionPack = true;
            }
        }
        else
        {
            if (FrontEndMenuManager.m_nStatsScrollDir)
            {
                if (FrontEndMenuManager.m_fStatsScrollSpeed != 0.0f)
                    FrontEndMenuManager.m_fStatsScrollSpeed = 0.0f;
                FrontEndMenuManager.m_nStatsScrollDir = false;
            }
            else
            {
                if (FrontEndMenuManager.m_fStatsScrollSpeed == 0.0f)
    ...
    case 56:
        if (bEnterInput)
        {
            if (FrontEndMenuManager.m_nDisplayVideoMode != FrontEndMenuManager.m_nPrefsVideoMode)
            {
                FrontEndMenuManager.m_nPrefsVideoMode = FrontEndMenuManager.m_nDisplayVideoMode;
                ((void(*)(int))0x745C70)(FrontEndMenuManager.m_nPrefsVideoMode);
        ...
        else
        {
            ...
            if (nArrowsInput > 0)
            {
                int nCurrentVidMode = FrontEndMenuManager.m_nDisplayVideoMode + 1;
                ...
                FrontEndMenuManager.m_nDisplayVideoMode = nCurrentVidMode;
            }
            else
            {
                int nCurrentVidMode = FrontEndMenuManager.m_nDisplayVideoMode - 1;
    ...
    case 44:
        if (bEnterInput)
        {
            if (FrontEndMenuManager.m_nPrefsAntiAliasingDisp != FrontEndMenuManager.m_nPrefsAntiAliasing)
            {
                FrontEndMenuManager.m_nPrefsAntiAliasing = FrontEndMenuManager.m_nPrefsAntiAliasingDisp;

                RwD3D9ChangeMultiSamplingLevels(FrontEndMenuManager.m_nPrefsAntiAliasing);
                ((void(*)(int))0x745C70)(FrontEndMenuManager.m_nPrefsVideoMode);
                FrontEndMenuManager.SaveSettings();
            }
        }
        else
        {
            ...
            if (nArrowsInput > 0)
            {
                int nCurrentMode = FrontEndMenuManager.m_nPrefsAntiAliasingDisp + 1;

                if (nCurrentMode > nNumMultiSamplingModes)
                    nCurrentMode = 1;

                FrontEndMenuManager.m_nPrefsAntiAliasingDisp = nCurrentMode;
            }
            else
            {
                int nCurrentMode = FrontEndMenuManager.m_nPrefsAntiAliasingDisp - 1;
...
void MobileFrontEnd::PrintRadioStationList() {
    ...
    do {
        ...
        FrontEndMenuManager.FrontEndSprites[v2].Draw(CRect(fPosX + (spacing), fPosY, fPosX + (spacing) + (size), fPosY + (size)), CRGBA(255, 255, 255, FrontEndMenuManager.m_nPrefsRadioStation == v2 ? 255 : 50));
...
void MobileFrontEnd::PrintLegend() {
   ...
   // if (FrontEndMenuManager.m_bPrefsShowLegends) {
